...
def reinit_model(llm_model, embedding_model, llm_history_len, no_remote_model, top_k,
                 history):
    try:
        llm_model_ins = shared.loaderLLM(llm_model, no_remote_model)
...
def add_vs_name(vs_name, chatbot):
    ...
    elif vs_name == "session":
        vs_status = "新建的知识库名称不能为session"
        chatbot = chatbot + [[None, vs_status]]
        return gr.update(visible=True), gr.update(visible=True), gr.update(visible=True), gr.update(
            visible=False), chatbot
    else:
    ...
    with gr.Tab("对话"):
        with gr.Row():
            ...
            with gr.Column(scale=5):
                mode = gr.Radio(["LLM 对话", "知识库问答", "带Session的对话"],
                                label="请选择使用模式",
                                value="知识库问答", )
